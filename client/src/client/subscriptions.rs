...
impl BlokliSubscriptionClient for BlokliClient {
    fn subscribe_native_balance(&self, address: String) -> Result<impl Stream<Item = Result<NativeBalance>>> {
        Ok(self
    ...
    fn subscribe_token_balance(&self, address: String) -> Result<impl Stream<Item = Result<HoprBalance>>> {
        Ok(self
