        ...
        self.nest_transaction(tx)
            ...
            .perform(|tx| {
                Box::pin(async move {
                    ...
                    // Note: We don't set the `id` field explicitly because it's auto-increment
                    // and will be generated by the database

                    let initial_row = chain_info::ActiveModel::new();
                    ChainInfo::insert(initial_row)
                        .exec_without_returning(tx.as_ref())
                        .await?;

                    let initial_row = node_info::ActiveModel::new();
