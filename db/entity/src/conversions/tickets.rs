            ...
            TicketBuilder::default()
                ...
                .win_prob(
                    value
                        ...
                        .try_into()
                        .map_err(|_| {
                            DbEntityError::ConversionError("invalid winning probability".into())
                        })?,
                )
                .channel_epoch(U256::from_be_bytes(&value.channel_epoch).as_u32())
                .challenge(response.to_challenge()?)
                .signature(value.signature.as_slice().try_into().map_err(|_| {
                    DbEntityError::ConversionError("invalid signature format".into())
                })?)
                .build_verified(
                    value.hash.as_slice().try_into().map_err(|_| {
                        DbEntityError::ConversionError("invalid ticket hash".into())
                    })?,
                )
                .map_err(|e| {
                    DbEntityError::ConversionError(format!("invalid ticket in the db: {e}"))
                })?
...
impl From<AcknowledgedTicket> for ticket::ActiveModel {
    fn from(value: AcknowledgedTicket) -> Self {
        ticket::ActiveModel {
            ...
            channel_epoch: Set(U256::from(value.verified_ticket().channel_epoch)
                .to_be_bytes()
                .to_vec()),
